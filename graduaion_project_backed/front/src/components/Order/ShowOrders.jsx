...
export default function ShowOrderss() {
    ...
    async function getOrders() {
        ...
        setOrders(
            orderData.sort((a, b) => a.Orders.localeCompare(b.Orders))
        );
    ...
    const onSearchSubmit = useCallback(async (term) => {
        if (term) {
            const o = Orders.filter((Orders) => (Orders.customerName.toLowerCase().includes(term)));
            setFilteredOrder(o);
        } else {
                ...
                <div class="table-responsive">
                    <div class="table-wrapper">
                        ...
                        <table class="table table-striped table-hover table-bOrdersed">
                            <thead>
                                <tr >
                                    <th><button className=' btn btn-success' onClick={SentAll}>All</button></th>

                                    {State.map(({ id, name }) => {
                                        return (


                                            <th><button className=' btn btn-success' onClick={() => SentId(id)}>{name}</button></th>
                        ...
                        <table class="table table-striped table-hover table-bOrdersed">
                            <thead>
                                <tr>
                                    ...
                                    <th>Governorat <i class="fa fa-sort"></i></th>
                                    <th>Status <i class="fa fa-sort"></i></th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredOrder.map(({ id, customerName, customerPhone, cost, cityName, stateName, statusName }) => {
                                    return (
                                        <tr>
                                            <td>{id}</td>
                                            <td>{customerName}</td>
                                            ...
                                            <td>
                                                <Link to={`/editOrder/${id}`} href="#" class="edit" title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></Link>
